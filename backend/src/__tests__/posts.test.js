...
describe("updating posts", () => {
  test("should update the specified property", async () => {
    ...
    expect(updatedPost.content).toEqual("some content changed");
  });
...
describe("creating posts", () => {
  test("with all parameters should succeed", async () => {
    ...
    // Convert the expected post to match the schema field names
    const expectedPost = {
      title: post.title,
      content: post.contents, // Note: schema uses 'content', not 'contents'
      tags: post.tags,
    };

    expect(foundPost).toEqual(expect.objectContaining(expectedPost));
